...
public:
    ...
    static LBFRegressor* GetInstance(){
        static LBFRegressor* instance;   //局部静态变量
        if(instance==NULL){
            instance = new LBFRegressor();
            instance->Load(lbfModelPath);
        }
        return instance;
    };
    static cv::CascadeClassifier* GetCascade(){
